...
- Proposal files: `policies/evaluator/examples/proposal*.json`
...
```bash
python policies/evaluator/evaluator.py \
  --policy policies/boundary-lexicographic-policy.json \
  --proposal policies/evaluator/examples/proposal_deny_safety.json
2) Run all proposals (summary)
bash
コードをコピーする
python policies/evaluator/run_all.py \
  --policy policies/boundary-lexicographic-policy.json \
  --examples-dir policies/evaluator/examples \
...
python policies/evaluator/run_all.py \
  --policy policies/boundary-lexicographic-policy.json \
  --expect-map policies/evaluator/expectations.json
...
Those belong to BOA/patterns/bindings/ or downstream system integrations.